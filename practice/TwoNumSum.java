 ...
 * 所以返回 [0, 1]
 * --------------------------------------
 * @ClassName: TwoNumSum.java
 * @Date: 2018/12/5 15:46
...
public class TwoNumSum {
    public static void main(String[] args) {
        ...
        int[] resultOne = twoSumOne(nums, target);
        System.out.println(Arrays.toString(resultOne));

        int[] resultTwo = twoSumTwo(nums, target);
        System.out.println(Arrays.toString(resultTwo));

        int[] resultThree = twoSumThree(nums, target);
     ...
     * 复杂度分析：
     * 时间复杂度：O(n^2)，对于每个元素，通过遍历数组的其余部分来寻找它所对应的目标元素，这将耗费 O(n) 的时间。因此时间复杂度为 O(n^2)。
     * 空间复杂度：O(1)。
